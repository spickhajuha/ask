...
if (likeBtn && likeCount) {
  async function updateLikeUI() {
    if (!currentUserId || !currentVideoId) return;
    const snap = await get(ref(db, `videos/${currentVideoId}/likedBy/${currentUserId}`));
    if (snap.exists()) {
...
if (commentForm && commentInput && commentList) {
  ...
  // ✅ Load comments with profile pic & name directly
  function loadComments(vid) {
    commentList.innerHTML = "";
    onValue(ref(db, `comments/${vid}`), (snap) => {
      commentList.innerHTML = "";
      const d = snap.val() || {};
      const entries = Object.values(d).sort((a, b) => a.createdAt - b.createdAt);
